...
public class Omnity : MonoBehaviour {
    ...
    public void UnloadOmnity() {
        ...
        Transform t = transform.Find("Omnity");
        if (t != null) {
    ...
    public void GenerateTransforms() {
        if (!omnityTransform) {
            omnityTransform = transform.Find("Omnity");
            if (!omnityTransform) {
        ...
        if (!transformRenderChannels) {
            transformRenderChannels = omnityTransform.Find("Render Channel Cameras");
            if (!transformRenderChannels) {
        ...
        transformScreenShapes = omnityTransform.Find("Screen Shapes");
        ...
        transformFinalPassCameras = omnityTransform.Find("Final Pass Cameras");
        ...
        transformTilted = omnityTransform.Find("Tilted");
        if (!transformTilted) {
        ...
        transformRenderChannelsTilted = omnityTransform.Find("Render Channel Cameras Tilted");
        ...
        transformScreenShapesTilted = omnityTransform.Find("Screenshapes Tilted");
        ...
        transformFinalPassCamerasTilted = omnityTransform.Find("Final Pass Cameras Tilted");
    ...
    public void DoUpdateShaders() {
        foreach (ScreenShape myScreenShape in screenShapes) {
            if (myScreenShape.startEnabled) {
                ...
                myScreenShape.SetJunkMatrices(automatic);
            }
        }
    }
    }
    ...
    private void Update() {
        if (!OmnityLoader.LoadingNow) {
            ...
            if (myOmnityGUI.GUIEnabled) {
                if (saveCursorState()) setCursorState(CursorLockMode.None, true);
            } else {
                restoreCursorState();
            }
    ...
    public bool FinalPassCameraArrayResize(int newsize, bool _serialize = true, bool _copyRenderTextureSettings = false, bool spawnNow = true, bool? followTilt = null) {
        newsize = Mathf.Max(0, newsize);
    ...
    public void ScreenShapeArrayResize(int newsize, bool _serialize = true, bool _automaticallyConnectTextures = true, OmnityScreenShapeType? _screenShapeType = null, FinalPassShaderType? _finalPassShaderType = null, bool spawnNow = true, bool? followTilt = null) {
        newsize = Mathf.Max(0, newsize);
            ...
            switch (projectorType) {
                case OmnityProjectorType.Rectilinear:
                    if (omnityPerspectiveMatrix.matrixMode == OmnityMono.MatrixMode.Orthographic) {
                        ...
                        myCamera.orthographic = false;
                        if (omnityPerspectiveMatrix.matrixMode == MatrixMode.HorizontalFOV) {
...
public class ScreenShape {
    ...
    private bool ShouldWeShowShaderFileName() {
        switch (finalPassShaderType) {
            ...
            case FinalPassShaderType.Equirectangular:
                return false;

            case FinalPassShaderType.LineShader:
                return false;

            case FinalPassShaderType.OmniEdgeBlendShaderAdd:
                return false;

            case FinalPassShaderType.OmniEdgeBlendShaderMultiply:
                return false;

            case FinalPassShaderType.OmniEdgeBlendShaderSubtract:
                return false;
            ...
            case FinalPassShaderType.EdgeBlendVertexColor:
                return false;

            case FinalPassShaderType.EdgeBlendVertexColorPerspective:
                return false;
    ...
    static public void WinResize(ref Rect WindowRect, ref bool resize_enabledX, ref bool resize_enabledY) {
        ...
        if (Input.GetMouseButtonDown(0) || resize_enable) {
            ...
            if (resize_enable != true) {
                if (Mathf.Abs(OmnityInputHelper.mousePosition.x - (WindowRect.x + WindowRect.width + radius / 4.0f)) < radius) {
                    resize_enabledX = true;
                    startClickPosition = OmnityInputHelper.mousePosition;
                    startSize = new Vector2(WindowRect.width, WindowRect.height);
                }
                if (Mathf.Abs(OmnityInputHelper.mousePosition.y - (Screen.height - WindowRect.y - WindowRect.height - radius / 4.0f)) < radius) {
        ...
        if (resize_enabledX) {
            WindowRect.width = OmnityInputHelper.mousePosition.x - startClickPosition.x + startSize.x;
        }
        if (resize_enabledY) {
            WindowRect.height = -(OmnityInputHelper.mousePosition.y - startClickPosition.y) + startSize.y;
        }
    }
    ...
    private static void DrawRect(ref Rect returnVal, ref int? mouseButtonDown) {
        try {
            ...
            if (mouseButtonDown != null) {
                float dx = OmnityInputHelper.mouseAxisX;
                float dy = OmnityInputHelper.mouseAxisY;
...
static public class OmnityInputHelper {
    static public System.Func<bool> fnMouseLeftPress = () => Input.GetMouseButtonDown(0);
    static public System.Func<float> fnMouseWheelX = () => UnityEngine.Input.mouseScrollDelta.x;
    static public System.Func<float> fnMouseWheelY = () => UnityEngine.Input.mouseScrollDelta.y;
    static public System.Func<Vector2> fnMousePosition = () => Input.mousePosition;
    static public System.Func<float> fnMouseAxisX = () => Input.GetAxis("Mouse X");
    static public System.Func<float> fnMouseAxisY = () => Input.GetAxis("Mouse Y");
    static public System.Func<int> fnMouseRemoteID = () => -1;
    static public bool mouseLeftPress { get { return fnMouseLeftPress(); } }
    static public float mouseWheelX { get { return fnMouseWheelX(); } }
    static public float mouseWheelY { get { return fnMouseWheelY(); } }
    static public Vector2 mousePosition { get { return fnMousePosition(); } } // OmnityInputHelper.mousePosition
    static public float mouseAxisX { get { return fnMouseAxisX(); } }
    static public float mouseAxisY { get { return fnMouseAxisY(); } }
    static public int mouseRemoteId { get { return fnMouseRemoteID(); } } // OmnityInputHelper.mouseRemoteId
}
