            ...
            if (ComputeNeeded() || _rayButtonUp == null) {
                switch (mouseType) {
                    ...
                    case OmnityMouseType.MouseDomeScreen:
                        goto case OmnityMouseType.MouseFlatScreen;
                    case OmnityMouseType.MouseFlatScreen:
                        _rayButtonUp = Input.GetMouseButtonUp(0);
                        break;
        ...
        get {
            if (ComputeNeeded() || _rayButtonDown == null) {
                switch (mouseType) {
                    ...
                    case OmnityMouseType.MouseFlatScreen:
                        _rayButtonDown = Input.GetMouseButtonDown(0);
                        break;
        ...
        get {
            if (ComputeNeeded() || null == _rayButton) {
                switch (mouseType) {
                    ...
                    case OmnityMouseType.MouseFlatScreen:
                        _rayButton = Input.GetMouseButton(0);
                        break;
                ...
                switch (mouseType) {
                    ...
                    case OmnityMouseType.MouseFlatScreen:
                        ...
                        try {
                            ...
                            if (cameraForFlat == null) {
                                ...
                                if (OmnityInputHelper.mouseRemoteId == -1) {
                                    if (cameraForFlat.name.ContainsCaseInsensitiveSimple("EZ3")) {
                                        onCorrectDisplay = false;
                                    } else {
                                        onCorrectDisplay = cameraForFlat.myCamera.pixelRect.Contains(OmnityInputHelper.mousePosition);
                                    }
                                } else {
                                    onCorrectDisplay = (OmnityInputHelper.mouseRemoteId == finalPassCameraIndex);
                                }
                            } else {
                                onCorrectDisplay = cameraForFlat.myCamera.pixelRect.Contains(OmnityInputHelper.mousePosition);
                            }
                            if (onCorrectDisplay) {
                                oldRay = cameraForFlat.myCamera.ScreenPointToRay(OmnityInputHelper.mousePosition);
                            }
                            _ray = oldRay;
                        } catch {
                        }
                    ...
                    case OmnityMouseType.MouseDomeScreen:
                        ...
                        if (cameraForDome == null || cameraForDome.myCamera == null) {
                            ...
                            onCorrectDisplay = cameraForDome.myCamera.pixelRect.Contains(OmnityInputHelper.mousePosition);
                        }
...
#else
                            Vector3 InputmousePosition = cameraForDome.myCamera.ScreenToViewportPoint(OmnityInputHelper.mousePosition);
